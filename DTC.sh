...
check_dpkg () {
	LC_ALL=C dpkg --list | awk '{print $2}' | grep "^${pkg}" 
>/dev/null || deb_pkgs="${deb_pkgs}${pkg} "
}
...
if [ ! "x${old_address}" = "x" ] ; then
	sed -i -e 
's:git.jdl.com/software:git.kernel.org/pub/scm/linux/kernel/git/jdl:g' 
${HOME}/git/${project}/.git/config
fi
